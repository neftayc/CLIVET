...
from ..forms.detallecompra import DetalleCompraForm
...
class DetalleCompraListView(ListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        """dispatch."""
        return super(DetalleCompraListView,
    ...
    def get_queryset(self):
        ...
        self.f = empty(self.request, 'f', 'producto')
...
class DetalleCompraCreateView(CreateView):
    ...
    template_name = "compras/detallecompra/form.html"
    success_url = reverse_lazy("compras:detallecompra_list")

    @method_decorator(permission_resource_required)
    def dispatch(self, request, *args, **kwargs):
        """dispatch."""
        return super(DetalleCompraCreateView,
                     self).dispatch(request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        Funcion con los primeros datos iniciales para la carga del template.
        """
        context = super(DetalleCompraCreateView,
                        self).get_context_data(**kwargs)
        context['opts'] = self.model._meta
        # context['cmi'] = 'tipodoc'
        context['title'] = ('Agregar %s') % ('DetalleCompra')
        return context
    ...
    def form_valid(self, form):
        ...
        return super(DetalleCompraCreateView, self).form_valid(form)
...
class DetalleCompraUpdateView(UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(DetalleCompraUpdateView,
                     self).dispatch(request, *args, **kwargs)

    def get_context_data(self, **kwargs):
        """Tipo Documento Update View context data."""
        context = super(DetalleCompraUpdateView,
                        self).get_context_data(**kwargs)
        context['opts'] = self.model._meta
        # context['cmi'] = 'empresa'
        context['title'] = ('Actualizar %s') % ('DetalleCompra')
        return context
    ...
    def form_valid(self, form):
        ...
        if self.object.id:
            messages.success(self.request, msg)
            log.warning(msg, extra=log_params(self.request))
        return super(DetalleCompraUpdateView, self).form_valid(form)
...
class DetalleCompraDeleteView(DeleteView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            ...
            log.warning(force_text(e), extra=log_params(self.request))
            return HttpResponseRedirect(self.success_url)
        return super(DetalleCompraDeleteView,
                     self).dispatch(request, *args, **kwargs)
